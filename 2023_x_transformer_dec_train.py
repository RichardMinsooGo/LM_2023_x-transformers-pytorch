...
if os.path.isfile('./checkpoints/2023_x_transformer.pt'):
    model.load_state_dict(torch.load('./checkpoints/2023_x_transformer.pt'))
...
for epoch in range(N_EPOCHS):
    ...
    if train_loss < best_epoch_loss:
        ...
        torch.save(model.state_dict(), './checkpoints/2023_x_transformer.pt')
...
trained_model = model = TransformerWrapper(
    num_tokens  = n_dec_vocab,
    max_seq_len = 1024,
    attn_layers = Decoder(
        dim     = 512,
...
trained_model.load_state_dict(torch.load('./checkpoints/2023_x_transformer.pt'))
